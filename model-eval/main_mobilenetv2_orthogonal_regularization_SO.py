...
def main_worker(gpu, ngpus_per_node, args):
    ...
    criterion_orig = nn.CrossEntropyLoss().cuda(args.gpu)

    optimizer, scheduler = optimizers.__dict__[args.optimizer](model)

    from torchvision.models import MobileNetV2
    from models import RepackagedResNet18

    def criterion(output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        # I suppose they will stay on the same device
        lam = 0.025
        orig_loss = criterion_orig(output, target)
        if type(model.module) is MobileNetV2:
            assert type(model.module.classifier) is nn.Identity
            w = model.module.features[18].weight.view(1000, 320)
            eye = torch.eye(320).cuda()
            orth_regularize = (torch.matmul(w.t(), w) - eye).norm()
        elif type(model.module) is RepackagedResNet18:
            w = model.module.features.layer4[1].conv2.weight.view(1000, -1)
            eye = torch.eye(1000).cuda()
            orth_regularize = (torch.matmul(w, w.t()) - eye).norm()
        else:
            raise NotImplementedError
        loss = orig_loss + lam * orth_regularize
        return loss
    ...
    # Data loading code
    train_dataset, val_dataset = datasets.__dict__[args.dataset]()

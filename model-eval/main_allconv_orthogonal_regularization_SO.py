...
def main_worker(gpu, ngpus_per_node, args):
    ...
    def criterion(output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        device = orig_loss.device
        orth_regularize = torch.tensor(0.).to(device)
        for m in model.modules():
            if type(m) is nn.Conv2d:
                w = m.weight.view(m.weight.size(0), -1)
                if w.size(0) > w.size(1):
                    eye = torch.eye(w.size(1)).to(device)
                    orth_regularize = orth_regularize + (torch.matmul(w.t(), w) - eye).norm()
                else:
                    eye = torch.eye(w.size(0)).to(device)
                    orth_regularize = orth_regularize + (torch.matmul(w, w.t()) - eye).norm()

...
def main_worker(gpu, ngpus_per_node, args):
    ...
    assert type(model.module) is MobileNetV2
    ...
    def criterion(output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        orth_regularize = (torch.matmul(w.t(), w) - torch.eye(320).cuda()).norm()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch == 20:
            if next(m.features[16].parameters()).requires_grad is False:
                m.module.requires_grad_(True)
